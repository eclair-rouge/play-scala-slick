...
trait Tables {
  ...
  lazy val schema: profile.SchemaDescription = People.schema ++ PlayEvolutions.schema
  ...
  case class PeopleRow(id: Long, name: String, age: Int)
  /** GetResult implicit for fetching PeopleRow objects using plain SQL queries */
  implicit def GetResultPeopleRow(implicit e0: GR[Long], e1: GR[String], e2: GR[Int]): GR[PeopleRow] = GR{
    prs => import prs._
    PeopleRow.tupled((<<[Long], <<[String], <<[Int]))
  }
  /** Table description of table people. Objects of this class serve as prototypes for rows in queries. */
  class People(_tableTag: Tag) extends profile.api.Table[PeopleRow](_tableTag, Some("study"), "people") {
    def * = (id, name, age) <> (PeopleRow.tupled, PeopleRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(name), Rep.Some(age))).shaped.<>({r=>import r._; _1.map(_=> PeopleRow.tupled((_1.get, _2.get, _3.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
  ...
  class PlayEvolutions(_tableTag: Tag) extends profile.api.Table[PlayEvolutionsRow](_tableTag, Some("study"), "play_evolutions") {
